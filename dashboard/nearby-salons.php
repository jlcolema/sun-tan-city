<?php include ($_SERVER['DOCUMENT_ROOT']."/a/inc/head.php"); ?>

<body id="dashboard" class="nearby-salons signed-in">

	<div id="page">

		<?php include ($_SERVER['DOCUMENT_ROOT']."/a/inc/header.php"); ?>

		<?php include ($_SERVER['DOCUMENT_ROOT']."/a/inc/nav.php"); ?>

		<div id="section" class="title-only">

			<div class="wrap">

				<div class="section">

					<h1>Nearby Salons</h1>

				</div>

			</div>

		</div>

		<div id="content">

			<div class="wrap">

				<div id="main" role="main">

					<a href="/dashboard/" class="k-button back-to no-background gray"><span class="k-icon">&larr;</span> Back To Dashboard</a>

					<p>I found the following locations within 3 miles of your favorite salon. Please pick one you wish to check in to and it will redirect you back to the check-in page.</p>

					<div class="suggested-salons">

						<div class="individual-location vcard">

							<div class="fn n org">Salon Name</div>

							<p class="overview">In the Walmart Supercenter at 86th and Michigan Ave.</p>

							<div class="panelbar">

								<div class="inner-wrap">

									<span class="k-link">More Salon Info</span>

									<div>

										<div class="group">

											<div class="tel-adr">

												<div class="tel"><b>Phone:</b> 913-780-8888</div>

												<div class="adr">

													<div class="street-address">14563 Westgate Rd.</div>

													<span class="locality">Indianapolis</span>,
													<span class="region">IN</span>,
													<span class="postal-code">46260</span>

												</div>

											</div>

											<div class="hours">

												<h3 class="title">Hours</h3>

												<ul>

													<li><span>M-F:</span> 8:00pm-9:00pm</li>
													<li><span>Sat-Sun:</span> 11:00pm-5:00pm</li>

												</ul>

											</div>

										</div>

										<div class="map">

											<iframe>

												Map

											</iframe>

										</div>

									</div>

								</div>

								<div class="select">

									<a href="/dashboard/" class="k-button">Change to this Salon</a>

								</div>

							</div>

							<div class="current-wait-times">

								<h3 class="title">Current Wait Times <span class="note">*</span></h3>

								<p>Select your desired bed type</p>

								<ol>

									<li>

										<select>

											<option>Sunless</option>
											<option>Option One</option>
											<option>Option Two</option>
											<option>Option Three</option>

										</select>

									</li>

									<li>

										<select>

											<option>Instant</option>
											<option>Option One</option>
											<option>Option Two</option>
											<option>Option Three</option>

										</select>

									</li>

									<li>

										<select>

											<option>Fastest</option>
											<option>Option One</option>
											<option>Option Two</option>
											<option>Option Three</option>

										</select>

									</li>

									<li>

										<select>

											<option>Faster</option>
											<option>Option One</option>
											<option>Option Two</option>
											<option>Option Three</option>

										</select>

									</li>

									<li>

										<select>

											<option>Fast</option>
											<option>Option One</option>
											<option>Option Two</option>
											<option>Option Three</option>

										</select>

									</li>

								</ol>

							</div>

						</div>

						<div class="individual-location vcard">

							<div class="fn n org">Salon Name</div>

							<p class="overview">In the Walmart Supercenter at 86th and Michigan Ave.</p>

							<div class="panelbar">

								<div class="inner-wrap">

									<span class="k-link">More Salon Info</span>

									<div>

										<div class="group">

											<div class="tel-adr">

												<div class="tel"><b>Phone:</b> 913-780-8888</div>

												<div class="adr">

													<div class="street-address">14563 Westgate Rd.</div>

													<span class="locality">Indianapolis</span>,
													<span class="region">IN</span>,
													<span class="postal-code">46260</span>

												</div>

											</div>

											<div class="hours">

												<h3 class="title">Hours</h3>

												<ul>

													<li><span>M-F:</span> 8:00pm-9:00pm</li>
													<li><span>Sat-Sun:</span> 11:00pm-5:00pm</li>

												</ul>

											</div>

										</div>

										<div class="map">

											<iframe>

												Map

											</iframe>

										</div>

									</div>

								</div>

								<div class="select">

									<a href="/dashboard/" class="k-button">Change to this Salon</a>

								</div>

							</div>

							<div class="current-wait-times">

								<h3 class="title">Current Wait Times <span class="note">*</span></h3>

								<p>Select your desired bed type</p>

								<ol>

									<li>

										<select>

											<option>Sunless</option>
											<option>Option One</option>
											<option>Option Two</option>
											<option>Option Three</option>

										</select>

									</li>

									<li>

										<select>

											<option>Instant</option>
											<option>Option One</option>
											<option>Option Two</option>
											<option>Option Three</option>

										</select>

									</li>

									<li>

										<select>

											<option>Fastest</option>
											<option>Option One</option>
											<option>Option Two</option>
											<option>Option Three</option>

										</select>

									</li>

									<li>

										<select>

											<option>Faster</option>
											<option>Option One</option>
											<option>Option Two</option>
											<option>Option Three</option>

										</select>

									</li>

									<li>

										<select>

											<option>Fast</option>
											<option>Option One</option>
											<option>Option Two</option>
											<option>Option Three</option>

										</select>

									</li>

								</ol>

							</div>

						</div>

						<div class="individual-location vcard">

							<div class="fn n org">Salon Name</div>

							<p class="overview">In the Walmart Supercenter at 86th and Michigan Ave.</p>

							<div class="panelbar">

								<div class="inner-wrap">

									<span class="k-link">More Salon Info</span>

									<div>

										<div class="group">

											<div class="tel-adr">

												<div class="tel"><b>Phone:</b> 913-780-8888</div>

												<div class="adr">

													<div class="street-address">14563 Westgate Rd.</div>

													<span class="locality">Indianapolis</span>,
													<span class="region">IN</span>,
													<span class="postal-code">46260</span>

												</div>

											</div>

											<div class="hours">

												<h3 class="title">Hours</h3>

												<ul>

													<li><span>M-F:</span> 8:00pm-9:00pm</li>
													<li><span>Sat-Sun:</span> 11:00pm-5:00pm</li>

												</ul>

											</div>

										</div>

										<div class="map">

											<iframe>

												Map

											</iframe>

										</div>

									</div>

								</div>

								<div class="select">

									<a href="/dashboard/" class="k-button">Change to this Salon</a>

								</div>

							</div>

							<div class="current-wait-times">

								<h3 class="title">Current Wait Times <span class="note">*</span></h3>

								<p>Select your desired bed type</p>

								<ol>

									<li>

										<select>

											<option>Sunless</option>
											<option>Option One</option>
											<option>Option Two</option>
											<option>Option Three</option>

										</select>

									</li>

									<li>

										<select>

											<option>Instant</option>
											<option>Option One</option>
											<option>Option Two</option>
											<option>Option Three</option>

										</select>

									</li>

									<li>

										<select>

											<option>Fastest</option>
											<option>Option One</option>
											<option>Option Two</option>
											<option>Option Three</option>

										</select>

									</li>

									<li>

										<select>

											<option>Faster</option>
											<option>Option One</option>
											<option>Option Two</option>
											<option>Option Three</option>

										</select>

									</li>

									<li>

										<select>

											<option>Fast</option>
											<option>Option One</option>
											<option>Option Two</option>
											<option>Option Three</option>

										</select>

									</li>

								</ol>

							</div>

						</div>

					</div>

					<div class="salon-search">

						<p>Care to find a different salon? You can search by city and state or zip code.</p>

						<?php include ($_SERVER['DOCUMENT_ROOT']."/a/inc/salon-search.php"); ?>

					</div>

					<p>

						<small>* Wait times are approximate and estimates only. Actual wait times will vary depending on level of activity and number of check-ins at the salon.</small>

					</p>

				</div>

			</div>

		</div>

		<div id="page-shim"></div>

	</div>

<?php include ($_SERVER['DOCUMENT_ROOT']."/a/inc/footer.php"); ?>